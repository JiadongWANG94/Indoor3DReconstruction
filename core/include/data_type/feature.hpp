...
class Feature {
 public:
    ...
    virtual sfmStatus_t GetPosition(Eigen::Vector2d *position) const = 0;
    virtual sfmStatus_t GetColor(Eigen::Vector3d *color) const = 0;
};

class SURFFeature : public Feature {
 public:
    SURFFeature(const cv::KeyPoint &key_point, const cv::Mat::Mat &descriptor, const Eigen::Vector3d &color) :
